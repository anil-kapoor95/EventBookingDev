...
if (isset($tpl['status']))
{
	if ($tpl['status'] == 'IP_BLOCKED') {
		?>
		<p class="text-danger text-center"><?php __('front_ip_address_blocked');?></p>
		<?php 
	} else {
		...
		<table style="width: 100%">
			...
			<tbody>
				...
				<tr>
					...
					<td><?php echo nl2br(stripslashes($tpl['arr']['description'])); ?></td>
				</tr>
				<?php
			...
			<tfoot>
				<tr>
					<td>&nbsp;</td>
					<td>
						<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjFrontPublic&amp;action=pjActionCancel" method="post">
